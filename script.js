...
const VISIBILITY_RADIUS_METERS = 2000; // 2km
...
function updateMarkersVisibility(playerLat, playerLon) {
    ...
    const layersToFilter = [
        { layer: enemyLayer, name: "Enemies", cache: enemies, idField: 'id', markerField: 'marker', latField: 'lat', lonField: 'lon' },
...
function calculatePlayerPower() {
    playerPower = playerStats.influence +
                  ...
                  playerStats.hitRate;
    // TODO: Update UI if power is displayed anywhere
...
function calculateCharacterStats() {
    ...
    // TODO: Update UI if these stats are displayed
}
...
// Note: inventoryListElement is now inside the inventory modal.
// We'll get this element inside the DOMContentLoaded listener where the modal logic resides.
// const inventoryListElement = document.getElementById('inventory-list'); // Get inventory list element (MOVED)
...
function saveUserInfo() {
    ...
    showCustomAlert('User info saved!'); // Use custom alert
...
function updateInventoryUI() {
    ...
    if (playerInventory.length === 0) {
        listElement.innerHTML = '<li></li>'; // Use empty list item for CSS styling
        return;
    }
...
async function fetchBasesInBounds(bounds) {
    ...
    console.log("Querying Overpass API for bases in bounds:", bounds);
     try {
        ...
        showCustomAlert("Could not fetch nearby bases. The service might be busy."); // Use custom alert
...
function joinOrganizationManually(orgName, orgAbbr, baseLat, baseLon) { // Renamed function and parameters
     ...
     if (!currentUserLocation) {
        showCustomAlert("Cannot determine your location."); // Use custom alert
        return;
    }
    if (currentUserOrganization) { // Use renamed variable
        showCustomAlert(`You are already in the ${currentUserOrganization.name}.`); // Use custom alert
        return;
    }
    ...
    if (distance <= MANUAL_JOIN_DISTANCE) {
        ...
        showCustomAlert(`You have joined the ${orgName}!`); // Use custom alert
        // TODO: Persist organization membership & base location
    } else {
         showCustomAlert(`You are too far away from this base to join ${orgName}! You need to be within ${MANUAL_JOIN_DISTANCE}m. (Distance: ${distance.toFixed(1)}m)`); // Use custom alert
    }
}
...
async function findAndJoinInitialOrganization(userLat, userLon) { // Renamed function
    ...
    if (!baseWithinManualRangeExists && closestBase) {
        ...
        updateOrganizationUI(); // Call renamed function
        updateBusinessMarkers(); // *** Explicitly update business icons/popups ***
        showCustomAlert(`No organizations found within ${MANUAL_JOIN_DISTANCE}m. You have been automatically assigned to the closest one: ${closestBase.organizationName}.`); // Use custom alert
        // TODO: Persist organization membership & base location
...
function collectCash(cashMarker, cashLat, cashLon) {
    ...
    if (distance <= CASH_COLLECTION_DISTANCE) {
        ...
        if (Math.random() < ITEM_DROP_CHANCE) {
            ...
            showCustomAlert(`You found a ${items[droppedItemId].name}!`); // Use custom alert
        } else {
            // Collect cash
            const amount = Math.floor(Math.random() * (cashDropData.maxAmount - cashDropData.minAmount + 1)) + cashDropData.minAmount;
            currentCash += amount;
            cashAmountElement.textContent = currentCash;
            showCustomAlert(`You collected $${amount}!`); // Use custom alert
            // TODO: Add transaction details to storage if needed
        }
    } else {
        // Too far away
        showCustomAlert(`You are too far away to collect this cash! (Distance: ${distance.toFixed(1)}m)`); // Use custom alert
    }
}
    ...
    for (let i = 0; i < numRivals; i++) {
        ...
        const marker = L.marker([lat, lon], { icon: rivalIcon }).addTo(rivalLayer) // Add to rivalLayer
            ...
            .on('click', () => {
                ...
                showCustomAlert(`You encountered ${randomRival.name}! Interaction not implemented yet.`); // Use custom alert
            });
    }
...
function activateProtection(businessId) {
    ...
    if (!currentUserOrganization) {
        showCustomAlert("You must be in an organization to protect a business."); // Use custom alert
        return;
    }
    ...
    if (distanceToBusiness > PROTECTION_ACTIVATION_RANGE) {
        showCustomAlert(`You are too far away from ${businessInfo.name} to activate protection. (Need to be within ${PROTECTION_ACTIVATION_RANGE}m, currently ${distanceToBusiness.toFixed(0)}m)`); // Use custom alert
        return;
    }
    ...
    if (businessInfo.protectingOrganization && businessInfo.protectingOrganization.abbreviation !== currentUserOrganization.abbreviation) {
        showCustomAlert(`${businessInfo.name} is already protected by ${businessInfo.protectingOrganization.name}. Contesting not implemented yet.`); // Use custom alert
    ...
    if (alreadyProtecting) {
        showCustomAlert(`You are already contributing protection to ${businessInfo.name}.`); // Use custom alert
        return;
    }
    ...
    if (businessInfo.protectingOrganization && businessInfo.protectingOrganization.abbreviation === currentUserOrganization.abbreviation && businessInfo.protectingUsers.length >= MAX_PROTECTING_USERS) {
        showCustomAlert(`${businessInfo.name} already has the maximum number of protectors (${MAX_PROTECTING_USERS}) from your organization.`); // Use custom alert
        return;
    }
     ...
     if (!businessInfo.protectingOrganization && businessInfo.protectingUsers.length >= MAX_PROTECTING_USERS) {
         showCustomAlert(`${businessInfo.name} already has the maximum number of protectors (${MAX_PROTECTING_USERS}).`); // Use custom alert
         return;
     }
    ...
    showCustomAlert(`You are now helping protect ${businessInfo.name}! Total Protection Power: ${businessInfo.protectionPower}`); // Use custom alert
...
function displayBusinesses(businesses) {
     businesses.forEach(businessInfo => {
        ...
        // Always use the custom business icon
        let icon = customBusinessIcon;
...
function handlePopupOpenForActions(e) {
    ...
    if (collectButton) {
        collectButton.classList.add('btn', 'btn-green'); // Add classes
    ...
    if (joinButton) {
        joinButton.classList.add('btn', 'btn-green'); // Add classes
    ...
    if (activateButton) {
        activateButton.classList.add('btn', 'btn-blue'); // Add classes
    ...
    if (interactEnemyButton) {
        interactEnemyButton.classList.add('btn', 'btn-orange'); // Add classes
        const interactHandler = function() {
            const enemyId = this.getAttribute('data-enemy-id');
            const enemy = findEnemyById(enemyId); // Function likely from enemy.js

            if (enemy) {
                // Prepare player stats object for the battle
                // Ensure player stats are up-to-date before battle
                calculateCharacterStats(); // Recalculate derived stats based on equipment etc.
                const currentPlayerBattleStats = {
                    name: playerAlias || playerUsername || "Player", // Use alias, username, or default
                    health: playerHealth, // Use the global playerHealth variable
                    // Use calculated stats which include equipment bonuses etc.
                    attack: playerStats.strength, // Example: Base attack from strength stat
                    defense: playerCharacterStats.defence // Use calculated defense
                    // Add other relevant stats if needed by battle.js (e.g., speed, accuracy)
                };
                ...
                // Check if initiateBattle function exists (it should if battle.js is loaded)
                if (typeof initiateBattle === 'function') {
                    // Initiate the battle
                    initiateBattle(currentPlayerBattleStats, enemy);
                } else {
...
function updateSingleBusinessMarker(businessId) {
     ...
     // Always use the custom icon
     const currentIcon = customBusinessIcon;
     let currentPopupContent = `<b>${businessInfo.name}</b><br>(${businessInfo.type})`;
     let isNowControlled = false; // For profit collection

     // --- Determine Profit Control Status (Icon no longer changes based on this) ---
     if (currentUserOrganization && currentOrganizationBaseLocation) {
         const distanceToBase = calculateDistance(
             businessInfo.lat, businessInfo.lon,
             currentOrganizationBaseLocation.lat, currentOrganizationBaseLocation.lon
         ...
         if (distanceToBase <= TERRITORY_RADIUS) {
             ...
             // Icon remains customBusinessIcon
         } else {
...
function collectProfit(businessId) {
    ...
    if (!currentUserLocation) {
     ...
     if (!currentUserOrganization || !businessInfo.isControlled) {
         showCustomAlert("This business is not under your organization's control."); // Use custom alert
         return;
    }
    ...
    if (distanceToBusiness <= PROFIT_COLLECTION_DISTANCE) {
        ...
        if (profit > 0) {
            ...
            showCustomAlert(`Collected $${profit} from ${businessInfo.name}.`); // Use custom alert
            ...
            showCustomAlert(`${businessInfo.name} has no profit to collect currently.`); // Use custom alert
        }
    } else {
        showCustomAlert(`You are too far away to collect profit from ${businessInfo.name}. (Distance: ${distanceToBusiness.toFixed(1)}m)`); // Use custom alert
    }
}
...
function leaveOrganization() { // Renamed function
    if (!currentUserOrganization) { // Use renamed variable
        showCustomAlert("You are not currently in an organization."); // Use custom alert
        return;
    }
    ...
    showCustomAlert(`You have left the ${orgName}.`); // Use custom alert
...
// updateInventoryUI(); // Removed initial call - will be updated when modal opens or items change
    ...
    if (containerDiv && minimizeBtn && showBtn) {
        ...
        showBtn.addEventListener('click', (e) => {
            ...
            if (containerId === 'protection-book' && !currentUserOrganization) {
                 ...
                 // showCustomAlert("Join an organization to view the Protection Book."); // Use custom alert
...
if (centerMapBtn) {
    centerMapBtn.addEventListener('click', () => {
        if (currentUserLocation) {
            ...
            showCustomAlert("Current location not available yet."); // Use custom alert
        }
    });
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (statsModal && openStatsBtn && closeStatsBtn) {
        ...
        console.error("Could not find all stats modal elements:", {
            modal: !!statsModal,
            openBtn: !!openStatsBtn,
    ...
    console.error("Could not find all equipment modal elements:", {
            modal: !!equipmentModal,
            openBtn: !!openEquipmentBtn,
            closeBtn: !!closeEquipmentBtn
        });
    }
    ...
    const inventoryListElement = document.querySelector('#inventory-modal #inventory-list'); // Get list inside modal
    ...
    if (inventoryModal && openInventoryBtn && closeInventoryBtn && inventoryListElement) {
        ...
        // TODO: Add click listener to inventoryListElement for item usage
        ...
        console.error("Could not find all inventory modal elements:", {
            modal: !!inventoryModal,
            openBtn: !!openInventoryBtn,
            closeBtn: !!closeInventoryBtn,
            list: !!inventoryListElement
        });
    }
...
function removeEnemyFromMap(enemyId) {
    ...
    if (markerElement) {
        markerElement.classList.add('marker-fade-out'); // Add fade-out class
